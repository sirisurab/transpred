...
def create_plot(df1: DataFrame, varcol1: str, label1: str, df2: DataFrame, varcol2: str, label2: str,
    ...
    if multiplot:
        df2 = df2.reset_index()
        g = sns.lineplot(x='date', y=varcol2, estimator=None, ax=ax1, hue=multicol, data=df2, kwargs={'linewidth':.7})
    else:
...
def create_reg_plot(df: DataFrame, varcol1: str, label1: str, varcol2: str, label2: str,
    ...
    if multiplot:
        g = sns.scatterplot(x=varcol1, y=varcol2, data=df, ax=ax, hue=multicol, kwargs={'s':10})
    else:
