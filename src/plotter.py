...
def plot_for_station(task: str, station: str, geomerged_cabs_df: DataFrame, geomerged_traffic_df: DataFrame, gas_df: DataFrame, weather_df: DataFrame):
    try:
        # for plotting
        ...
        if dolocationids.size > 0:
            ...
            ycabs_df = ycabs_df.set_index(cabs_datecols, 'weight').groupby(Grouper(freq='D', level=0)).sum().loc[start_date: end_date]
            print(gcabs_df.head())
            print(ycabs_df.head())
...
def plot(*args) -> bool:
    ...
    for station in inputs[1:]:
        p = Process(target=plot_for_station, kwargs={'task': task,
                                                     'station': station,
                                                     'geomerged_cabs_df': geomerged_cabs_df,
                                                     'geomerged_traffic_df': geomerged_traffic_df,
                                                     'gas_df': gas_df,
