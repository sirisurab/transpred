    ...
    if 'dolocationid' in df.columns:
        ...
        try:
            if ('dolatitude' in df.columns) and ('dolongitude' in df.columns):
                ...
                print('taxi zones GeoDF '+taxi_zone_df.head(2))
                ...
                print('converted df to GeoDF '+geodf.head(2))
                geodf = sjoin(geodf, taxi_zone_df, how='left', op='within')
                print('after spatial join with taxi zones '+geodf.head(1))
                #df = geodf[['dodatetime', 'LocationID', 'passengers']].rename({'LocationID':'dolocationid'})
                print('converted back to dataframe '+df.head(1))
                return df
            else:
